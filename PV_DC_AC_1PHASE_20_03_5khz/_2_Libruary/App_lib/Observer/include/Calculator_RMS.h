...
#define CALCULATOR_RMS_V2_CLA_MARCO(v)                                              \
    if(v.nsamples >= v.nsamplesTB)                                                  \
    {                                                                               \
        v.inv_sqrt_nsamples = __meisqrtf32(v.nsamples);                             \
        v.rms = __meinvf32(__meisqrtf32(v.sum_rms))*v.inv_sqrt_nsamples;            \
...
#define CALCULATOR_RMS_V3_CLA_MARCO(v)                                              \
    if(v.nsamples >= v.nsamplesTB)                                                  \
    {                                                                               \
        v.inv_sqrt_nsamples = CLAsqrt(v.nsamples);                                  \
        v.rms = (CLAsqrt(v.sum_rms))/v.inv_sqrt_nsamples;                           \
        v.sum_rms = 0;                                                              \
        v.nsamples=0;                                                               \
      }                                                                             \
    else                                                                            \
